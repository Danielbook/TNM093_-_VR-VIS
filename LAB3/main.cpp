...
void myInitOGLFun()
{
	...
	sun = new sgct_utils::SGCTSphere(0.2f, 512);
	earth = new sgct_utils::SGCTSphere(0.02f, 512);
	moon = new sgct_utils::SGCTSphere(0.05f, 512);
...
// 		}
	...
	for(size_t i = 0; i < sharedTransforms.getSize(); i++)
	{
		if(i != sharedHeadSensorIndex.getVal()) 
		{
			...
			glm::vec4 v1 = glm::abs( c - sharedTransforms.getValAt( i ) );
...
void myEncodeFun()
{
	...
	sgct::SharedData::instance()->writeFloat( &tid );
}
...
void myDecodeFun()
{
	...
	sgct::SharedData::instance()->readFloat( &tid );
}
